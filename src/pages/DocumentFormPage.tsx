...
const DocumentFormPage = () => {
  ...
  useEffect(() => {
    const fetchSubProjects = async () => {
      const response = await sousProjetService.getSousProjetById(document.id_projet, 'chef');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        // Prepare a plain object for update if your API does not support FormData for update
        const documentData = {
          titre: document.titre,
          type: document.type,
          description: document.description,
          date_ajout: document.date_ajout,
          chemin: '', // Set the file path or URL after uploading the file if needed
          id_projet: document.id_projet || '',
          id_sous_projet: document.id_sous_projet || '',
          nom_projet: document.nom_projet || '',
          nom_sous_projet: document.nom_sous_projet || ''
        };
        await documentService.updateDocument(editId, documentData, 'employee');
        toast({
